...
{
    [Authorize]
    public class GenericMedicineController : Controller
    {
        private readonly IGenericMedicineService _service;

        public GenericMedicineController(IGenericMedicineService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Upsert(GenericMedicineViewModel model)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> QuickAdd([FromBody] QuickAddGenericMedicineViewModel model)
        {
            if (!ModelState.IsValid)
